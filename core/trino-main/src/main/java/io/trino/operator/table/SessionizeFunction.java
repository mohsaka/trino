...
import static io.trino.operator.table.SessionizeFunction.SessionizeFunctionSplit.MAX_SPLIT_SIZE;
...
public class SessionizeFunction
        extends AbstractConnectorTableFunction
{
    public static final String NAME = "sessionize";
            ...
            ConnectorAccessControl accessControl)
    {
        ...
        if (start < stop && step <= 0) {
            throw new TrinoException(INVALID_FUNCTION_ARGUMENT, format("Step must be positive for sessionize [%s, %s]", start, stop));
        }

        if (start > stop && step >= 0) {
            throw new TrinoException(INVALID_FUNCTION_ARGUMENT, format("Step must be negative for sessionize [%s, %s]", start, stop));
        }

        return TableFunctionAnalysis.builder()
                .handle(new SessionizeFunctionHandle(start, stop, start == stop ? 0 : step))
                .build();
    }

    public record SessionizeFunctionHandle(long start, long stop, long step)
            implements ConnectorTableFunctionHandle
    {}

    public static ConnectorSplitSource getSessionizeFunctionSplitSource(SessionizeFunctionHandle handle)
    {
        ...
        if (step.equals(BigInteger.ZERO)) {
            ...
            return new FixedSplitSource(ImmutableList.of(new SessionizeFunctionSplit(start.longValueExact(), stop.longValueExact())));
        }

        ImmutableList.Builder<SessionizeFunctionSplit> splits = ImmutableList.builder();
        ...
        for (BigInteger i = BigInteger.ZERO; i.compareTo(totalSplits) < 0; i = i.add(BigInteger.ONE)) {
            ...
            splits.add(new SessionizeFunctionSplit(splitStart.longValueExact(), splitStop.longValueExact()));
    ...
    public static class SessionizeFunctionSplit
            implements ConnectorSplit
    {
        private static final int INSTANCE_SIZE = instanceSize(SessionizeFunctionSplit.class);
        public static final int DEFAULT_SPLIT_SIZE = 1000000;
        public static final int MAX_SPLIT_SIZE = 1000000;

        // the first value of sub-sessionize
        private final long start;

        // the last value of sub-sessionize. this value is aligned so that it belongs to the sessionize.
        private final long stop;

        @JsonCreator
        public SessionizeFunctionSplit(@JsonProperty("start") long start, @JsonProperty("stop") long stop)
        {
            this.start = start;
            this.stop = stop;
        }
    ...
    public static TableFunctionProcessorProvider getSessionizeFunctionProcessorProvider()
    {
        return new TableFunctionProcessorProvider()
        {
            ...
            {
                return new SessionizeFunctionProcessor(((SessionizeFunctionHandle) handle).step(), (SessionizeFunctionSplit) split);
            }
        };
    }

    public static class SessionizeFunctionProcessor
            implements TableFunctionSplitProcessor
    {
        ...
        public SessionizeFunctionProcessor(long step, SessionizeFunctionSplit split)
        {
            this.step = step;
